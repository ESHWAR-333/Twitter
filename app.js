...
app.get("/user/tweets/", authenticateToken, async (request, response) => {
  ...
  const getUserIdQuery = `select user_id from user where username='${username}';`;
  const userId = await db.get(getUserIdQuery);
  const tweetQuery = `select tweet_id from tweet where user_id='${userId.user_id}';`;
  const tweetResponse = await db.all(tweetQuery);
  //   console.log(tweetResponse);
  const tweet = tweetResponse.map((each) => {
    console.log(each.tweet_id);
    return each.tweet_id;
  });
  const likesQuery = `select tweet.tweet as tweet, count(like.tweet_id) as likes ,count(reply.tweet_id) as replies , tweet.date_time as dateTime from ((like inner join reply on like.user_id=reply.user_id) as t join tweet on t.user_id=tweet.user_id) where tweet.tweet_id in (${tweet});`;
  //   const likesQuery = `select * from tweet where tweet_id in (${tweet});`;
  const likesResponse = await db.all(likesQuery);
  response.send(likesResponse);
  //   const getTweetIds = likesResponse.map((eachId) => {
  //     return {
  //       tweet: eachId.tweet,
  //       likes: eachId.likes,
  //       replies: eachId.replies,
  //       dateTime: eachId.dateTime,
  //     };
  //   });
